...
class Helper
{
     ...
     *
     * Common usage:
     * Get items of a first menu level (to display on top of the site), e.g., $result = \Construct\Menu\Helper::getMenuItems('menu1', 1, 1);
     * Get 7 levels of children of selected page on top menu (to display on a side): $result = \Construct\Menu\Helper::getMenuItems('menu1', 2,  7);
     *
    ...
    public static function getMenuItems($menuName, $depthFrom = 1, $depthTo = 1000, $orderBy = null)
    {
        ...
        if ($depthFrom < 1) {
            ...
            if (isset($backtrace[0]['file']) && $backtrace[0]['line']) {
                throw new \Construct\Exception(
                    '$depthFrom can\'t be less than one. (Error source: ' . $backtrace[0]['file'] . ' line: ' . $backtrace[0]['line'] . ' ) '
                );
            } else {
                throw new \Construct\Exception ('$depthFrom can\'t be less than one.');
            }
        }
        ...
        if ($depthTo < $depthFrom) {
            ...
            if (isset($backtrace[0]['file']) && $backtrace[0]['line']) {
                ...
                throw new \Construct\Exception('$depthTo can\'t be lower than $depthFrom.');
            }
        }
        // end variable check
    ...
    private static function arrayToMenuItem($pages, $depth, $curDepth, $order)
    {
        ...
        foreach ($pages as $pageRow) {
            $page = new \Construct\Page($pageRow['id']);
            $item = new Item();
            $subSelected = false;
            if ($curDepth < $depth) {
            ...
            if ($page->isCurrent() || $page->getRedirectUrl() && $page->getLink(
                ) == \Construct\Internal\UrlHelper::getCurrentUrl()
            ) {
