...
class Dispatcher
{
    ...
    {
        $this->eventDispatcher = new \Construct\Internal\Dispatcher\EventDispatcher();
        $this->jobDispatcher = new \Construct\Internal\Dispatcher\JobDispatcher();
        $this->filterDispatcher = new \Construct\Internal\Dispatcher\FilterDispatcher();
        $this->slotDispatcher = new \Construct\Internal\Dispatcher\SlotDispatcher();
    }
    ...
    {
        // Parse event files:
        $coreModules = \Construct\Internal\Plugins\Model::getModules();
        foreach ($coreModules as $module) {
            $this->bindPluginEvents($module, '\Construct\Internal');
        }
    ...
    private function bindPluginEventType($plugin, $type, $namespace)
    {
        ...
        foreach ($methods as $method) {
            if ($method->isStatic()) {
                ...
                throw new \Construct\Exception(esc("{$plugin}\\{$type}::{$method->getName()} must be static."));
            }
        }
        return null;
    }
