    ...
    public function validate($values, $valueKey, $environment)
    {
        ...
        if (preg_match('/^#([a-f]|[A-F]|[0-9]){3}(([a-f]|[A-F]|[0-9]){3})?$\b/', $values[$valueKey])) {
            ...
            if ($environment == \Construct\Form::ENVIRONMENT_ADMIN) {
                return __('Incorrect color code', 'Construct-admin', false);
            } else {
                return __('Incorrect color code', 'Construct', false);
            }
        }
    }
