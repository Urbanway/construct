    ...
    public function validate($values, $valueKey, $environment)
    {
        if (isset($values[$valueKey]['file']) && is_array($values[$valueKey]['file'])) {
            foreach ($values[$valueKey]['file'] as $file) {
                $uploadModel = \Construct\Internal\Repository\UploadModel::instance();
                if (!$uploadModel->isFileUploadedByCurrentUser($file, true)) {
                    if ($environment == \Construct\Form::ENVIRONMENT_ADMIN) {
                        $error = __('Session has ended. Please remove and re-upload files.', 'Construct-admin', false);
                    } else {
                        $error = __('Session has ended. Please remove and re-upload files.', 'Construct', false);
                    }
