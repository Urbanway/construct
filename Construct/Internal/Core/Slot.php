...
class Slot
{
    public static function breadcrumb_80($params)
    {
        ...
        return \Construct\Internal\Breadcrumb\Service::generateBreadcrumb($showHome);
    }
    ...
    public static function languages_80($params)
    {
        ...
        return ipView('Construct/Internal/Config/view/languages.php', $data);
    }

    public static function logo_80()
    {
        $inlineManagementService = new \Construct\Internal\InlineManagement\Service();
    ...
    public static function menu_80($params)
    {
        $data = array(
            ...
            'parent' => 'parent',
            'children' => 'children',
            'view' => 'Construct/Internal/Config/view/menu.php'
        );
        ...
        if (!empty($params[0]) && is_object($params[0]) && $params[0] instanceof \Construct\Menu\Item) {
            $params = array(
                'items' => $params
            );
        }
        ...
        if (isset($params['items']) && is_string($params['items'])) {
            $data['items'] = \Construct\Menu\Helper::getMenuItems($params['items']);
        }
        if (empty($data['attributes']) || !is_array($data['attributes'])) {
            $data['attributes'] = [];
        }
    ...
    public static function text_80($params)
    {
        ...
        $attributesStr = '';
        if (empty($params['id'])) {
            throw new \Construct\Exception("Construct.text slot requires parameter 'id'");
        }
        $key = $params['id'];
    ...
    public static function image_80($params)
    {
        ...
        $cssClass = '';
        if (empty($params['id'])) {
            throw new \Construct\Exception("Construct.image slot requires parameter 'id'");
        }
        $key = $params['id'];
