...
class Model
{
    ...
    /**
     * @param \Construct\Request $request
     * @return bool
     */
    public static function isLoginPage(\Construct\Request $request = null)
    {
        if ($request == null) {
            $request = ipRequest();
        }
    ...
    public static function isPasswordResetPage(\Construct\Request $request = null)
    {
        if ($request == null) {
            $request = ipRequest();
        }
     ...
     * @return \Construct\Internal\Admin\MenuItem[]
     */
    public function getAdminMenuItems($currentModule)
    {
        ...
        $modules = \Construct\Internal\Plugins\Model::getModules();
        foreach ($modules as $module) {

            // skipping modules that don't have 'index' (default) action in AdminController
            $controllerClass = 'Construct\\Internal\\' . $module . '\\AdminController';
            if (!class_exists($controllerClass) || !method_exists($controllerClass, 'index')) {
                continue;
            }

            $moduleItem = new \Construct\Internal\Admin\MenuItem();
            $moduleItem->setTitle(__($module, 'Construct-admin', false));
        ...
        $plugins = \Construct\Internal\Plugins\Service::getActivePlugins();
        ...
        foreach ($plugins as $plugin) {
            ...
            $moduleItem = new \Construct\Internal\Admin\MenuItem();
            $moduleItem->setTitle(__($plugin['title'], 'Construct-admin', false));
    ...
    {
        $conf = \Construct\Internal\Plugins\Service::getPluginConfig($module);
    ...
    public function login($username, $password)
    {
        ...
        $administrator = \Construct\Internal\Administrators\Service::getByUsername($username);
        if (!$administrator) {
            \Construct\ServiceLocator::dispatcher()->event(
                'ipAdminLoginFailed',
                array('username' => $username, 'ip' => ipRequest()->getServer('REMOTE_ADDR'))
            );
            ipLog()->info('Admin.incorrectLogin: {username} from {ip}', array('username' => $username, 'ip' => $ip));
            $this->errors = array('login' => __('Following user doesn\'t exist', 'Construct-admin'));
            return false;
        }

        if (\Construct\Internal\Administrators\Service::checkPassword($administrator['id'], $password)) {
            ...
            ipLog()->info('Admin.incorrectLogin: {username} from {ip}', array('username' => $username, 'ip' => $ip));
            $this->errors = array('password' => __('Incorrect password', 'Construct-admin'));
            return false;
        }
    }
