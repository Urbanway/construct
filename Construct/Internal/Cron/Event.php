...
namespace Construct\Internal\Cron;
...
class Event
{
    public static function ipBeforeApplicationClosed()
    {
        ...
        */
        if (!ipConfig()->database() || \Construct\Internal\Admin\Model::isSafeMode() || !ipGetOption('Config.automaticCron', 1)) {
            return;
        }

        $lastExecution = \Construct\ServiceLocator::storage()->get('Cron', 'lastExecutionStart');
        ...
        $lastFailureAt = \Construct\ServiceLocator::storage()->get('Cron', 'lastFailureAt');
        ...
        if (function_exists('curl_init')) {
            ...
            if ($fakeCronAnswer != __('OK', 'Construct-admin', false)) {
            ...
            $fakeCronAnswer = \Construct\ServiceLocator::application()->handleRequest($request)->getContent();
