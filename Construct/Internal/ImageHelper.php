...
class ImageHelper
{
    ...
    public static function resize($imageFile, $widthDest, $heightDest, $destDir, $type, $forced, $quality)
    {
        ...
        if (!self::resizeRequired($imageInfo[0], $imageInfo[1], $widthDest, $heightDest, $type, $forced)) {
            $newName = \Construct\Internal\File\Functions::genUnoccupiedName($imageFile, $destDir);
        ...
        try {
            ...
            throw new \Construct\Exception($e->getMessage(), $e->getCode(), $e);
        }
    ...
    public static function crop($imageFile, $destDir, $x1, $y1, $x2, $y2, $quality, $widthDest, $heightDest)
    {
        ...
        if (!self::getMemoryNeeded($imageFile)) {
            throw new \Construct\Exception("Can't get memory needed", self::ERROR_MEMORY);
        }
        ...
        if ($x2 - $x1 > imagesx($image) || $y2 - $y1 > imagesy(
            ...
            $newName = \Construct\Internal\File\Functions::genUnoccupiedName($tmpImageName, $destDir);
        } else {
            $sx1 = $x1;
            $sx2 = $x2;
            $sy1 = $y1;
    ...
    public static function getMemoryNeeded($imageFile)
    {
        ...
        return \Construct\Internal\System\Helper\SystemInfo::allocateMemory($bytesNeeded);
    }
