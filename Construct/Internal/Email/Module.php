    ...
    function addEmail($from, $fromName, $to, $toName, $subject, $email, $immediate, $html, $files = null)
    {
        ...
        if ($files) {
            ...
            foreach ($files as $fileSetting) {
                ...
                $new_name = \Construct\Internal\File\Functions::genUnoccupiedName($new_name, ipFile('file/tmp/'));
                if (copy($file['real_name'], ipFile('file/tmp/' . $new_name))) {
                    $cached_files[] = ipFile('file/tmp/' . $new_name);
                    $cached_fileNames[] = $file['required_name'];
                    $tmpMimeType = \Construct\Internal\File\Functions::getMimeType($file['real_name']);
                    if ($tmpMimeType == null) {
            ...
            if ($locked) {
                ...
                foreach ($emails as $key => $email) {
                    ...
                    if ($email['html']) {
                        ...
                        try {
                            $altBody = \Construct\Internal\Text\Html2Text::convert($email['email']);
                        } catch (\Construct\Internal\Text\Html2TextException $e) {
                            $altBody = $email['email'];
                        }
                        $mail->AltBody = $altBody;
                    } else {
                        /*$h2t = new \Construct\Internal\Text\Html2Text($content, false);
