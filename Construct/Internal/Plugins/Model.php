...
class Model
{
    ...
    public static function activatePlugin($pluginName)
    {
        ...
        if (!$config) {
            throw new \Construct\Exception(ipFile(
                'Plugin/' . esc($pluginName) . '/Setup/plugin.json'
            ) . ' doesn\'t exist or is incorrect');
        }

        if (empty($config['name']) || $config['name'] !== $pluginName) {
            throw new \Construct\Exception('Plugin name setting in ' . ipFile(
                'Plugin/' . esc($pluginName) . '/Setup/plugin.json'
            ) . " doesn't match the folder name.");
        }

        if (empty($config['version'])) {
            throw new \Construct\Exception('Missing plugin version number in ' . ipFile(
                'Plugin/' . esc($pluginName) . '/Setup/plugin.json'
            ) . " file.");
        }

        if (!empty($pluginRecord['version']) && (float)$pluginRecord['version'] > (float)$config['version']) {
            throw new \Construct\Exception\Plugin\Setup("You can't downgrade the plugin. Please remove the plugin completely and reinstall if you want to use older version.");
        }
        ...
        ipLog()->info(
            'Construct.pluginActivated: {plugin} {version} activated.',
    ...
    public static function removePlugin($pluginName)
    {
        $activePlugins = self::getActivePluginNames();
        if (in_array($pluginName, $activePlugins)) {
            throw new \Construct\Exception\Plugin\Setup('Please deactivate the plugin before removing it.');
        ...
        try {
            ...
            if (!$result) {
                throw new \Construct\Exception\Plugin\Setup('Can\'t remove folder ' . esc($pluginDir));
            }
        } catch (\Construct\PhpException $e) {
            throw new \Construct\Exception\Plugin\Setup('Can\'t remove folder ' . esc($pluginDir));
        }

        ipLog()->info(
            'Construct.pluginRemoved: {plugin} {version} removed.',
    ...
    {
        if (ipGetOption('Construct.disablePluginMarket', 0)) {
            return '';
        }
