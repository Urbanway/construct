...
-----END PUBLIC KEY-----';
    ...
    public function downloadPlugin($name, $url, $signature)
    {
        ...
        if (!$pluginTempFilename) {
            throw new \Construct\Exception('Plugin file download failed.');
        }
        ...
        if (!$verified) {
            throw new \Construct\Exception('Plugin signature verification failed.');
        }

        //extract
        $secureTmpDir = ipFile('file/secure/tmp/');
        $tmpExtractedDir = \Construct\Internal\File\Functions::genUnoccupiedName($name, $secureTmpDir);
        \Construct\Internal\Helper\Zip::extract($secureTmpDir . $pluginTempFilename, $secureTmpDir . $tmpExtractedDir);
        unlink($archivePath);

        //install
        $extractedDir = $this->getFirstDir($secureTmpDir . $tmpExtractedDir);
        $installDir = Model::pluginInstallDir();
        $newPluginDir = \Construct\Internal\File\Functions::genUnoccupiedName($name, $installDir);
