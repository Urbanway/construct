...
use Construct\Response;
use Construct\Response\Redirect;
...
class PublicController extends \Construct\Controller
{
    ...
    public function index()
    {
        ...
        $response->setLayout(ipFile('Construct/Internal/Install/view/layout.php'));
        return $response;
    }
    ...
    public function testConfiguration()
    {
        ...
        if (!empty($errors)) {
            return \Construct\Response\JsonRpc::error(__('Please correct errors.', 'Install', false))->addErrorData('errors', $errors);
        }

        return \Construct\Response\JsonRpc::result(array('redirect' => 'index.php?step=2'));
    }
    ...
    public function createDatabase()
    {
        ...
        foreach (array('hostname', 'username', 'database') as $key) {
            if (empty($db[$key])) {
                ...
                return \Construct\Response\JsonRpc::error(__('Please fill in required fields.', 'Install', false));
            }
        }
        ...
        if (strlen($db['tablePrefix']) > 7) {
            ...
            return \Construct\Response\JsonRpc::error(__("Prefix can't be longer than 7 symbols.", 'Install', false));
        }

        if ($db['tablePrefix'] != '' && !preg_match('/^([A-Za-z_][A-Za-z0-9_]*)$/', $db['tablePrefix'])) {
            $_SESSION['db_errors'][] = 'Prefix is bad';
            return \Construct\Response\JsonRpc::error(__("Prefix can't contain any special characters and should start with a letter.", 'Install', false));
        }
        ...
        try {
            ...
            return \Construct\Response\JsonRpc::error(__('Specified database does not exists and cannot be created.', 'Install', false));
        }

        if (Helper::testDBTables($db['tablePrefix']) && empty($db['replaceTables'])) {
            $_SESSION['db_errors'][] = 'Replace tables';
            return \Construct\Response\JsonRpc::error(__('Do you like to replace existing tables in the database?', 'Install', false), 'table_exists');
        }
        ...
        if ($errors) {
            ...
            return \Construct\Response\JsonRpc::error(__('There were errors while executing install queries. ' . serialize($errors), 'Install', false));
        }
        ...
        try {
            ...
            ipStorage()->set('Construct', 'cachedBaseUrl', $cachedBaseUrl);
            ipStorage()->set('Construct', 'websiteId', $_SESSION['websiteId']);
            ipStorage()->set('Construct', 'getconstructSupport', $_SESSION['config']['support']);
        } catch (\Exception $e) {
            $_SESSION['db_errors'][] = $e->getTraceAsString();
            return \Construct\Response\JsonRpc::error($e->getTraceAsString());
        }
        ...
        try {
            ...
            return \Construct\Response\JsonRpc::error(__('Can\'t write configuration "/config.php"', 'Install', false));
        }
