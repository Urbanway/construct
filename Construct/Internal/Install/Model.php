...
class Model
{
    ...
    public static function createAndUseDatabase($database)
    {
        ...
        try {
            ...
            try {
                ...
                throw new \Construct\Exception('Could not create database');
            }
        }
    ...
    {
        $sql = file_get_contents(self::ipFile('Construct/Internal/Install/sql/structure.sql'));
    ...
    public static function importData($tablePrefix)
    {
        ...
        $sqlFile = self::ipFile('Construct/Internal/Install/sql/data.sql');
        $fh = fopen($sqlFile, 'r');
        ...
        $sql = str_replace("[[[[dbversion]]]]", \Construct\Internal\Update\Model::getDbVersion(), $sql);
    ...
    public static function writeConfigFile($config, $filename)
    {
        $configInfo = array(
            ...
            'rewritesDisabled' => array(
                ...
                'comment' => 'Install has not detected mod_rewrite. Delete this line if mod_rewrite (http://construct.uws.al/help/mod-rewrite) is enabled or you are using Nginx and have made required changes to the configuration file (http://construct.uws.al/help/nginx).'
            ),
            // END GLOBAL
 ...
 */
 ...
 return [" . $configCode . "\n];";
    ...
    {
        $adminId = \Construct\Internal\Administrators\Service::add($user, $email, $pass);
