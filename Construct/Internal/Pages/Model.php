...
class Model
{
    ...
    {
        $oldRevision = \Construct\Internal\Revision::getPublishedRevision($sourceId);
        \Construct\Internal\Revision::duplicateRevision($oldRevision['revisionId'], $targetId, 1);
    }
    ...
    public static function movePage($pageId, $destinationParentId, $destinationPosition)
    {
        if ((int)$pageId === (int)$destinationParentId || static::isChild($destinationParentId, $pageId)) {
            throw new \Construct\Exception("Can't move page inside itself.");
        }
    ...
    public static function createMenu($languageCode, $alias, $title, $type = 'tree')
    {
        ...
        if (empty($alias)) {
            $alias = \Construct\Internal\Text\Specialchars::url($title);
        }
