 ...
 * @package Construct\Internal\Design
 *
...
-----END PUBLIC KEY-----';
    ...
    public function __construct()
    {
        if (!defined('IP_PHPSECLIB_DIR')) {
            define('IP_PHPSECLIB_DIR', ipFile('Construct/Lib/phpseclib/'));
        }
    ...
    public function downloadTheme($name, $url, $signature)
    {
        ...
        if (!$themeTempFilename) {
            throw new \Construct\Exception('Theme file download failed.');
        }
        ...
        if (!$verified) {
            throw new \Construct\Exception('Theme signature verification failed.');
        }

        //extract
        $helper = Helper::instance();
        $secureTmpDir = ipFile('file/secure/tmp/');
        $tmpExtractedDir = \Construct\Internal\File\Functions::genUnoccupiedName($name, $secureTmpDir);

        \Construct\Internal\Helper\Zip::extract($secureTmpDir . $themeTempFilename, $secureTmpDir . $tmpExtractedDir);
        unlink($archivePath);

        //install
        $extractedDir = $helper->getFirstDir($secureTmpDir . $tmpExtractedDir);
        $installDir = $model->getThemeInstallDir();
        $newThemeDir = \Construct\Internal\File\Functions::genUnoccupiedName($name, $installDir);
