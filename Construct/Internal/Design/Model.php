...
class Model
{
    ...
    public function getThemePlugins()
    {
        ...
        foreach ($plugins as $plugin) {
            ...
            if (is_dir(
                ...
                $pluginDir .= '/';
                $pluginConfiguration = \Construct\Internal\Plugins\Service::parsePluginConfigFile($pluginDir);
    ...
    public function installThemePlugin($pluginName)
    {
        ...
        if (is_dir($toDir)) {
            throw new \Construct\Exception('This plugin has been already installed');
        }

        if (!is_dir($fromDir)) {
            throw new \Construct\Exception('Plugin is missing.');
        }

        $pluginConfiguration = \Construct\Internal\Plugins\Service::parsePluginConfigFile($fromDir);

        if (!$pluginConfiguration) {
            throw new \Construct\Exception('Can\'t read plugin configuration file.');
        }

        if (!is_writable(ipFile('Plugin/'))) {
            throw new \Construct\Exception('Please make plugin dir writable (' . esc($this->getThemePluginDir()) . ')');
        }

        $helper = Helper::instance();
        $helper->cpDir($fromDir, $toDir);
        \Construct\Internal\Plugins\Service::activatePlugin($pluginName);
    }
    ...
    public function installTheme($themeName)
    {
        ...
        if (!isset($themes[$themeName])) {
            throw new \Construct\Exception("Theme '" . esc($themeName) . "' does not exist.");
        }
        ...
        \Construct\ServiceLocator::storage()->set('Construct', 'theme', $themeName);
    ...
    {
        if (ipGetOption('Construct.disableThemeMarket', 0)) {
            return '';
        }
    ...
    public function getTheme($name, $dir = null, $url = null)
    {
        ...
        if (!empty($config['doctype']) && defined('\Construct\View::' . $config['doctype'])) {
