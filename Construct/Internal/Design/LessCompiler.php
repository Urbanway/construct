...
class LessCompiler
{
    ...
    public function compileFile($themeName, $lessFile)
    {
        ...
        try {
            require_once ipFile('Construct/Lib/less.php/Less.php');
            $themeDir = ipFile('Theme/' . $themeName . '/assets/');
            $ipContentDir = ipFile('Construct/Internal/Core/assets/ipContent/');
    ...
    {
        $lessFiles = glob(ipFile('Theme/' . $themeName . '/' . \Construct\Application::ASSETS_DIR . '/') . '*.less');
    ...
    public function rebuild($themeName)
    {
        ...
        foreach ($lessFiles as $file) {
            ...
            file_put_contents(
                ipFile(
                    'Theme/' . $themeName . '/' . \Construct\Application::ASSETS_DIR . '/' . substr($lessFile, 0, -4) . 'css'
                ),
                $css
            );
        }
