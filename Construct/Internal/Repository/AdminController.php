...
class AdminController extends \Construct\Controller
{
    ...
    public function storeNewFiles()
    {
        ...
        if (!isset($post['files']) || !is_array($post['files'])) {
            return new \Construct\Response\Json(array('status' => 'error', 'errorMessage' => 'Missing POST variable'));
        }
        ...
        foreach ($files as $file) {
            ...
            $newName = \Construct\Internal\File\Functions::genUnoccupiedName($file['renameTo'], $destination);
            copy($source, $destination . $newName);
            unlink($source); //this is a temporary file
            $browserModel = \Construct\Internal\Repository\BrowserModel::instance();
    ...
    public function addFromUrl()
    {
        ...
        if (!ipAdminPermission('Repository upload')) {
            throw new \Construct\Exception("Permission denied");
        }

        if (!isset($_POST['files']) || !is_array($_POST['files'])) {
            throw new \Construct\Exception('Invalid parameters.');
        }
        $files = $_POST['files'];
    ...
    protected function downloadFile($url, $title)
    {
        ...
        $net = new \Construct\Internal\NetHelper();
        ...
        try {
            $mime = \Construct\Internal\File\Functions::getMimeType(ipFile('file/tmp/' . $tmpFilename));
            switch ($mime) {
                case 'image/png':
                    $ext = '.jpg';
                    break;
        ...
        $title = \Construct\Internal\File\Functions::cleanupFileName($title);
        $words = explode(' ', $title);
        $cleanTitle = '';
        ...
        $destinationFileName = \Construct\Internal\File\Functions::genUnoccupiedName($niceFileName, $destinationDir);
