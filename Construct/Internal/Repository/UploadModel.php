...
class UploadModel
{
     ...
     * @throws \Construct\Exception\Repository\Upload
     */
    public function handlePlupload($secureFolder)
    {
        if (!$secureFolder && !ipAdminId()) {
            throw new \Construct\Exception\Repository\Upload("Trying to upload image to temporary directory without permission.");
        }
        ...
        if ($secureFolder) {
            ...
            if ($this->folderSize($targetDir) > $sizeLimit * 1000000) { //4000 Mb by default
                ...
                throw new \Construct\Exception("Upload limit reached");
            }
        ...
        $fileName = \Construct\Internal\File\Functions::cleanupFileName($fileName);

        // Make sure the fileName is unique but only if chunking is disabled
        if ($chunks < 2 && file_exists($targetDir . $fileName)) {
            $fileName = \Construct\Internal\File\Functions::genUnoccupiedName($fileName, $targetDir);
        }
        ...
        if (!empty($fileExtension) && !in_array($fileExtension, $whiteListExtensions)) {
            //security risk
            throw new \Construct\Exception\Repository\Upload\ForbiddenFileExtension("Files with extension (." . esc(
                $fileExtension
        ...
        if (strpos($contentType, "multipart") !== false) {
            if (!isset($_FILES['file']['tmp_name']) || !is_uploaded_file($_FILES['file']['tmp_name'])) {
                throw new \Construct\Exception\Repository\Upload("Failed to move uploaded file.");
            }

            // Open temp file
            $out = fopen($targetDir . $fileName, $chunk == 0 ? "wb" : "ab");
            if (!$out) {
                throw new \Construct\Exception\Repository\Upload("Failed to open output stream.");
            }
            ...
            if (!$in) {
                throw new \Construct\Exception\Repository\Upload("Failed to open input stream.");
            }
    ...
    public function getUploadedFilePath($fileName, $secure)
    {
        if (!$this->isFileUploadedByCurrentUser($fileName, $secure)) {
            throw new \Construct\Exception\Repository\Upload("This user didn't upload this file or session has ended.");
        }
