...
class BrowserModel
{
    ...
    public function pathMustBeInRepository($path, $secure)
    {
        ...
        if ($secure) {
            if (strpos($relpath, realpath(ipFile('file/secure/'))) !== 0) {
                throw new \Construct\Exception("Restricted directory");
            }
        } else {
     ...
     * @param string $secure
     * @return array
     * @throws \Construct\Exception\Repository
     */
    ...
    private function getFileData($fileName, $secure, $subdir = null)
    {
        ...
        if (!file_exists($file) || !is_file($file)) {
            throw new \Construct\Exception\Repository("File doesn't exist " . esc($file));
        }
    ...
    private function createPreview($file)
    {
        ...
        return ipFileUrl('Construct/Internal/Repository/assets/icons/general.png');
    }
