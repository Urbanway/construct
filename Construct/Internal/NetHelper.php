...
class NetHelper
{
    ...
    {
        if (!$forceFilename) {
            $desiredFilename = \Construct\Internal\File\Functions::genUnoccupiedName($desiredFilename, $destinationDir);
        }
    ...
    public function fetchUrl($uri)
    {
        ...
        if ($httpCode != 200) {
            throw new \Construct\Exception('Could not fetch uri', array('httpCode' => $httpCode));
        }
