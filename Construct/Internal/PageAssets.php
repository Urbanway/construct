...
class PageAssets
{
    ...
    public function generateHead()
    {
        ...
        if (!empty($data['ipDesign']['pCfg']) || !empty($data['restoreDefault'])) {
            $inDesignPreview = \Construct\Internal\Design\ConfigModel::instance()->isInPreviewState();
        }

        if ($inDesignPreview) {
            $themeAssetsUrl = ipThemeUrl(\Construct\Application::ASSETS_DIR . '/');
            $designService = \Construct\Internal\Design\Service::instance();
            ...
            foreach ($cssFiles as &$file) {
                if (strpos($file['value'], $themeAssetsUrl) === 0) {
                    ...
                    if ($pathinfo['extension'] == 'css'
                        ...
                        $themeFile = \Construct\Application::ASSETS_DIR . '/' . $pathinfo['filename'] . '.less';
        ...
        $head = ipView('Construct/Internal/Config/view/head.php', $data)->render();
    ...
    public function generateJavascript()
    {
        ...
        $data = array(
            'ip' => array(
                ...
                'safeMode' => \Construct\Internal\Admin\Service::isSafeMode(),
                ...
                'securityToken' => \Construct\ServiceLocator::application()->getSecurityToken(),
        ...
        $javascript = ipView('Construct/Internal/Config/view/javascript.php', $data)->render();
    ...
    {
        return \Construct\ServiceLocator::storage()->get('Construct', 'cacheVersion', 1);
    }

    protected function getCurrentRevision()
    {
        return \Construct\ServiceLocator::content()->getCurrentRevision();
    }
