...
class Event
{
    protected static function addWidgetAssets(\Construct\WidgetController $widget)
    {
        $pluginAssetsPath = $widget->getWidgetDir() . \Construct\Application::ASSETS_DIR . '/';
    ...
    public static function ipBeforeController()
    {
        ...
        if (ipIsManagementState()) {
            ...
            ipAddJsVariable('ipPublishTranslation', __('Publish', 'Construct-admin', false));
        }
    }
    ...
    public static function ipCronExecute($info)
    {
        if ($info['firstTimeThisDay'] || $info['test']) {
            if (ipGetOption('Config.removeOldRevisions', 0)) {
                \Construct\Internal\Revision::removeOldRevisions(ipGetOption('Config.removeOldRevisionsDays', 720));
            }
        }
    }
