...
use Construct\Internal\Browser;
...
class Model
{
    ...
    public static function generateBlock($blockName, $revisionId, $languageId, $managementState, $exampleContent = '')
    {
        ...
        foreach ($widgets as $widget) {
            try {
                ...
                throw new \Construct\Exception\Content('Error when generating widget preview', null, $e);
            }
        }
    ...
    public static function initManagementData()
    {
        ...
        __('Other', 'Construct-admin', false);
        __('Core', 'Construct-admin', false);
    ...
    public static function generateWidgetPreviewFromStaticData($widgetName, $data, $skin = null)
    {
        ...
        if (!$widgetObject) {
            ...
            throw new \Construct\Exception\Content('Widget ' . esc(
                $widgetName
    ...
    private static function _generateWidgetPreview($widgetRecord, $managementState)
    {
        ...
        if ($managementState) {
            ...
            if (count($skins) > 1) {
                $optionsMenu[] = array(
                    'title' => __('Skin', 'Construct-admin', false),
                    'attributes' => array(
                        'class' => 'ipsSkin',
    ...
    /**
     * @return \Construct\WidgetController[]
     */
    ...
    public static function getWidgetObject($widgetName)
    {
        ...
        if (isset($widgetObjects[$widgetName])) {
            ...
            return new \Construct\Internal\Content\Widget\Missing\Controller('Missing', 'Content', true);
        }
    }
    ...
    public static function createWidget(
        ...
        $row = array(
            'data' => json_encode(\Construct\Internal\Text\Utf8::checkEncoding($data)),
            'skin' => $skin,
            'name' => $widgetName,
            'revisionId' => $revisionId,
            'languageId' => $languageId,
    ...
    public static function isRevisionModified($revisionId = null)
    {
        ...
        $currentRevision = \Construct\Internal\Revision::getRevision($revisionId);
        if (!$currentRevision) {
            return false;
        }
        $pageId = $currentRevision['pageId'];

        $publishedRevision = \Construct\Internal\Revision::getPublishedRevision($pageId);
