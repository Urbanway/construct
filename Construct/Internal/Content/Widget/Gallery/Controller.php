 ...
 *
 */
namespace Construct\Internal\Content\Widget\Gallery;
...
class Controller extends \Construct\WidgetController
{
    ...
    {
        return __('Gallery', 'Construct-admin', false);
    }
            ...
            switch ($postData['method']) {
                case 'move':
                    ...
                    if (!isset($currentData['images'][$originalPosition])) {
                        throw new \Construct\Exception("Moved image doesn't exist");
                    }
                ...
                case 'add':
                    ...
                    foreach ($postData['images'] as $image) {
                        ...
                        \Construct\Internal\Repository\Model::bindFile($image['fileName'], 'Content', $widgetId);
                ...
                case 'update' :
                    ...
                    if ($tmpData['imageOriginal'] != $postData['fileName']) {
                        ...
                        \Construct\Internal\Repository\Model::bindFile($postData['fileName'], 'Content', $widgetId);
                ...
                case 'saveSettings':
                    ...
                    break;
                default:
                    throw new \Construct\Exception('Unknown command');
    ...
    private function _deleteOneImage($image, $widgetId)
    {
        ...
        if (isset($image['imageOriginal']) && $image['imageOriginal']) {
            \Construct\Internal\Repository\Model::unbindFile($image['imageOriginal'], 'Content', $widgetId);
        }
    }
    ...
    public function duplicate($oldId, $newId, $data)
    {
        ...
        foreach ($data['images'] as $image) {
            ...
            if (isset($image['imageOriginal']) && $image['imageOriginal']) {
                \Construct\Internal\Repository\Model::bindFile($image['imageOriginal'], 'Content', $newId);
            }
        }
        return $data;
    }
    ...
    protected function linkForm()
    {
        ...
        $field = new \Construct\Form\Field\Select(
            array(
                'name' => 'type',
                'label' => __('Mouse click action', 'Construct-admin', false),
            ));

        $values = array(
            array('lightbox', __('Lightbox', 'Construct-admin', false)),
            array('link', __('URL', 'Construct-admin', false)),
            array('none', __('None', 'Construct-admin', false)),
        );
        ...
        $field = new \Construct\Form\Field\Checkbox(
            array(
                'name' => 'nofollow',
                'label' => __('Set rel="nofollow" attribute', 'Construct-admin', false),
            ));
        $form->addField($field);
    ...
    public function optionsMenu($revisionId, $widgetId, $data, $skin)
    {
        $answer = [];
        $answer[] = array(
            'title' => __('Add image', 'Construct-admin', false),
            'attributes' => array(
                'class' => 'ipsAdd'
            )
        );
        $answer[] = array(
            'title' => __('Manage images', 'Construct-admin', false),
            'attributes' => array(
                'class' => 'ipsManage'
            )
        );
