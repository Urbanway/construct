 ...
 *
 */
namespace Construct\Internal\Content\Widget\File;
...
class Controller extends \Construct\WidgetController{
    ...
    public function getTitle() {
        return __('File', 'Construct-admin', false);
    }
                    ...
                    switch($file['status']){
                        case 'new':
                            if (file_exists(ipFile('file/repository/' . $file['fileName']))) {
                                ...
                                \Construct\Internal\Repository\Model::bindFile($file['fileName'], 'Content', $widgetId);
                        ...
                        case 'deleted':
                            ...
                            if (!$existingFile) {
                                \Construct\Internal\Repository\Model::unbindFile($existingFile['fileName'], 'Content', $widgetId);
                            } else {
    ...
    public function delete($widgetId, $data) {
        ...
        foreach($data['files'] as $file) {
            if (isset($file['fileName']) && $file['fileName']) {
                \Construct\Internal\Repository\Model::unbindFile($file['fileName'], 'Content', $widgetId);
            }
        };
    }
    ...
    */
    public function duplicate($oldId, $newId, $data) {
        ...
        foreach($data['files'] as $file) {
            if (isset($file['fileName']) && $file['fileName']) {
                \Construct\Internal\Repository\Model::bindFile($file['fileName'], 'Content', $newId);
            }
        };
        return $data;
    }
