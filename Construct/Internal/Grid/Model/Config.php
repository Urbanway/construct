...
class Config
{
    ...
    public function __construct($config)
    {
        ...
        if (empty($this->config['table'])) {
            throw new \Construct\Exception('\'table\' configuration value missing.');
        }
    ...
    public function deleteWarning()
    {
        ...
        return __('Are you sure you want to delete?', 'Construct-admin', false);
    }
     ...
     * @return \Construct\Internal\Grid\Model\Field
     */
    public function fieldObject($field)
    {
        ...
        $class = '\\Construct\\Internal\\Grid\\Model\\Field\\' . $field['type'];
        if (!class_exists($class)) {
            if (class_exists($field['type'])) {
                $class = $field['type']; //type is full class name
            } else {
                throw new \Construct\Exception('Class doesn\'t exist "' . esc($field['type']) . '"');
            }
    ...
    public function layout()
    {
        if (empty($this->config['layout'])) {
            return 'Construct/Internal/Grid/view/layout.php';
        }
    ...
    protected function checkConfig(&$config, $depth = 1, $gridBreadcrumb = [])
    {
        ...
        if (!is_array($fields)) {
            throw new \Construct\Exception('GRID configuration is missing \'fields\' attribute for table ' . $config['table']);
        }
        ...
        if ($fields[0]['type'] != 'Tab') {
            ...
            if ($tabExist) {
                array_unshift($fields, array('label' => __('General', 'Construct-admin', false), 'type' => 'Tab'));
            }
    ...
    public function subgridConfig($statusVariables, $depthLimit = null)
    {
        ...
        for ($i = 1; $i < $depth; $i++) {
            ...
            if (!$found) {
                throw new \Construct\Exception('Unknown subgrid');
            }
        }
