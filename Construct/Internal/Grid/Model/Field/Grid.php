...
class Grid extends \Construct\Internal\Grid\Model\Field
{
    protected $config = [];
    ...
    public function __construct($fieldConfig, $wholeConfig)
    {
        ...
        if (empty($fieldConfig['gridId'])) {
            throw new \Construct\Exception('Grid field needs \'gridId\' - a unique identificator of a subgrid.');
        }
        $this->gridId = $fieldConfig['gridId'];

        if (empty($fieldConfig['config'])) {
            throw new \Construct\Exception('Grid field needs \'config\' setting to be set.');
        }
