...
class RepositoryFile extends \Construct\Internal\Grid\Model\Field
{
    protected $field = '';
    protected $label = '';
    ...
    {
        $field = new \Construct\Form\Field\RepositoryFile(array(
            'label' => $this->label,
            'name' => $this->field,
            'layout' => $this->layout,
    ...
    public function afterDelete($recordId, $curData)
    {
        if (!empty($curData[$this->field])) {
            \Construct\Internal\Repository\Model::unbindFile($curData[$this->field], $this->repositoryBindKey, $recordId);
        }
    }
    ...
    {
        if (!empty($curData[$this->field])) {
            \Construct\Internal\Repository\Model::bindFile($curData[$this->field][0], $this->repositoryBindKey, $recordId);
        }
    }
    ...
    public function afterUpdate($recordId, $oldData, $newData)
    {
        ...
        if (!empty($oldData[$this->field]) && $oldData[$this->field] != $newData[$this->field]) {
            \Construct\Internal\Repository\Model::unbindFile($oldData[$this->field], $this->repositoryBindKey, $recordId);
        }

        if (!empty($newData[$this->field]) && $oldData[$this->field] != $newData[$this->field]) {
            \Construct\Internal\Repository\Model::bindFile($newData[$this->field][0], $this->repositoryBindKey, $recordId);
        }
    }
