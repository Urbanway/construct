...
class Table extends \Construct\Internal\Grid\Model
{
    ...
    public function handleMethod()
    {
        ...
        if (empty($request['method'])) {
            throw new \Construct\Exception('Missing request data');
        }
    ...
    protected function page($params)
    {
        ...
        if (empty($params['page'])) {
            throw new \Construct\Exception('Missing parameters');
        }
    ...
    protected function update($data)
    {
        if (empty($data[$this->subgridConfig->idField()])) {
            throw new \Construct\Exception('Missing parameters. Most likely \'AUTO_INCREMENT\' attribute is missing on the database id field');
        }
     ...
     * @return \Construct\Internal\Grid\Model\Transformation
    ...
    protected function createTransformationObject($transformationSetting)
    {
        ...
        if (is_object($transformationSetting)) {
            if (!$transformationSetting instanceof \Construct\Internal\Grid\Model\Transformation) {
                throw new \Construct\Exception('Transformation object has to implement Construct\Internal\Grid\Model\Table\Transformation interface');
            }
            return $transformationSetting;
        }
        if (is_string($transformationSetting)) {
            if (strpos($transformationSetting, '\\') === false) {
                $transformationSetting = 'Construct\\Internal\\Grid\\Model\\Transformation\\' . $transformationSetting;
            }
    ...
    protected function subgrid($params)
    {
        if (empty($params['gridId'])) {
            throw new \Construct\Exception('girdId GET variable missing');
        }
        if (empty($params['gridParentId'])) {
            throw new \Construct\Exception('girdParentId GET variable missing');
        }
