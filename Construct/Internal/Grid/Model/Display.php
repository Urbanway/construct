...
class Display
{
    ...
    public function fullHtml()
    {
        ...
        $pagination = new \Construct\Pagination\Pagination(array(
    ...
    protected function getActions()
    {
        ...
        if ($this->subgridConfig->allowSearch()) {
            $actions['search'] = array(
                'label' => __('Search', 'Construct-admin', false),
                'class' => 'ipsSearch'
            );
        }
            ...
            foreach ($this->subgridConfig->fields() as $fieldData) {
                ...
                if (!empty($fieldData['preview']) && $fieldData['preview'] !== true) {
                    if (is_callable($fieldData['preview'])) {
                        ...
                        throw new \Construct\Exception('Field \'preivew\' value must be PHP callable or string');
                    }
    ...
    public function updateForm($id)
    {
        ...
        foreach ($this->subgridConfig->fields() as $key => $fieldData) {
            ...
            if (!empty($fieldData['type']) && $fieldData['type'] == 'Tab') {
                ...
                if ($key == 0) {
                    ...
                    $fieldset = new \Construct\Form\Fieldset($title);
        ...
        $field = new \Construct\Form\Field\Hidden(array(
        ...
        $field = new \Construct\Form\Field\HiddenSubmit();
        $form->addField($field);
    ...
    public function moveForm()
    {
        ...
        $field = new \Construct\Form\Field\Integer(array(
                'name' => 'position',
                'value' => ''
            ));
        $form->addField($field);
