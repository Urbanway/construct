...
class FileSystem
{
    ...
    public function createWritableDir($dir)
    {
        if (substr($dir, 0, 1) != '/' && $dir[1] != ':') { // Check if absolute path: '/' for unix, 'C:' for Windows
            throw new \Construct\Internal\Update\UpdateException('Absolute path required');
        }
    ...
    function makeWritable($path, $permissions = null)
    {
        ...
        if (!is_writable($path)) {
            ...
            $oldErrorHandler = set_error_handler(array('Construct\Internal\Update\Helper\FileSystem', 'handleError'));

            try {
                $originalConstructErrorHandler = set_error_handler('Construct\Internal\ErrorHandler::ipSilentErrorHandler');
                chmod($path, $permissions);
                set_error_handler($originalConstructErrorHandler);
    ...
    public function rm($dir)
    {
        ...
        set_error_handler($originalConstructErrorHandler);
        if (!is_writable($dir)) {
            throw new \Construct\Internal\Update\UpdateException("Directory is not writable: " . $dir);
        }
     ...
     * @var string $dir
     * @throws \Construct\Internal\Update\UpdateException
     */
    public function clean($dir)
    {
        if (!file_exists($dir) || !is_dir($dir)) {
            throw new \Construct\Internal\Update\UpdateException("Directory doesn't exist: " . $dir);
        }
    ...
    public function cpContent($source, $dest)
    {
        if (!is_dir($source) || !is_dir($dest)) {
            throw new \Construct\Internal\Update\UpdateException("Source or destination is not a folder. Source: " . $source . ". Destination: " . $dest . "");
        }
    ...
    {
        throw new \Construct\Internal\Update\UpdateException("Can't write directory " . $dir);
    }
    ...
    {
        throw new \Construct\Internal\Update\UpdateException($errstr);
    }
