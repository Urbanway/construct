...
use Construct\Internal\Content\Model;
...
class WidgetController
{
    ...
    public function __construct($name, $pluginName, $core = false)
    {
        ...
        if ($this->core) {
            ...
            $this->widgetDir = 'Construct/Internal/' . $pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/';
        } else {
            $this->widgetDir = 'Plugin/' . $pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/';
        }

        $this->widgetAssetsDir = $this->widgetDir . \Construct\Application::ASSETS_DIR . '/';
    }
    ...
    /**
     * Check if the widget is native construct widget
     *
    ...
    public function getIcon()
    {
        ...
        return ipFileUrl('Construct/Internal/Content/assets/img/iconWidget.svg');
    }
    ...
    public function getSkins()
    {
        ...
        if (!is_dir($skinDir)) {
            throw new \Construct\Exception\Content('Skins directory does not exist. ' . esc($skinDir));
        }
        ...
        if ($this->isCore()) {
            $themeViewsFolder = ipThemeFile(
                \Construct\View::OVERRIDE_DIR . '/Construct/Internal/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR
            );
        } else {
            $themeViewsFolder = ipThemeFile(
                \Construct\View::OVERRIDE_DIR . '/Plugin/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR
            );
        }
        ...
        foreach ($views as $viewKey => $view) {
            if ($this->isCore()) {
                $translation = __(ucfirst($viewKey), 'Construct-admin', false);
            } else {
        ...
        if (empty($skins)) {
            throw new \Construct\Exception\Content('No skins');
        }
     ...
     *
     * Use return new \Construct\Response\Json($jsonArray) to return JSON.
     *
    ...
    {
        $snippetDir = ipFile($this->getWidgetDir() . \Construct\Internal\Content\Model::SNIPPET_DIR) . '/';
    ...
    public function generateHtml($revisionId, $widgetId, $data, $skin)
    {
        ...
        try {
            if ($this->core) {
                $skinFile = 'Construct/Internal/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR . '/' . $skin . '.php';
            } else {
                $skinFile = 'Plugin/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR . '/' . $skin . '.php';
            }
            if (!is_file(ipFile($skinFile)) && !is_file(ipThemeFile(\Construct\View::OVERRIDE_DIR . '/' . $skinFile))) {
                $skin = 'default';
                if ($this->core) {
                    $skinFile = 'Construct/Internal/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR . '/' . $skin . '.php';
                } else {
