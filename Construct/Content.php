...
class Content
{
     ...
     *
     * @return \Construct\Language[] All website languages. Each element is a Language object.
     *
     */
    public function getLanguages()
    {
        if ($this->languages === null) {
            if (!ipConfig()->database()) {
                ...
                $languages = \Construct\Internal\Languages\Service::getLanguages();
                $this->languages = array();
                foreach ($languages as $data) {
                    $this->languages[] = \Construct\Internal\Content\Helper::createLanguage($data);
                }
            }
        }
    ...
    {
        return new \Construct\Block($blockName);
    }
    ...
    public function getCurrentRevision()
    {
        ...
        if (!$revision && ipIsManagementState()) {
            $revision = \Construct\Internal\Revision::getLastRevision($pageId);
            if ($revision['isPublished']) {
                $duplicatedId = \Construct\Internal\Revision::duplicateRevision($revision['revisionId']);
                $revision = \Construct\Internal\Revision::getRevision($duplicatedId);
            }
        }

        if (!$revision) {
            $revision = \Construct\Internal\Revision::getPublishedRevision($this->currentPage->getId());
        }
    ...
    public function getBreadcrumb($pageId = null)
    {
        ...
        if ($page) {
            ...
            while (!empty($parentPageId)) {
                $parentPage = new \Construct\Page($parentPageId);
                $pages[] = $parentPage;
    ...
    {
        $languageId = \Construct\Internal\Languages\Service::addLanguage(
            $title,
            $abbreviation,
            $code,
            $url,
    ...
    {
        \Construct\Internal\Languages\Service::delete($languageId);
    }
    ...
    {
        \Construct\Internal\Pages\Service::updatePage($pageId, $data);
    }
    ...
    {
        $newPageId = \Construct\Internal\Pages\Service::addPage($parentId, $title, $data);
        return $newPageId;
    }
    ...
    {
        $pageId = \Construct\Internal\Pages\Service::copyPage($pageId, $destinationParentId, $destinationPosition);
        return $pageId;
    }
    ...
    {
        \Construct\Internal\Pages\Service::movePage($pageId, $destinationParentId, $destinationPosition);
    }
    ...
    {
        \Construct\Internal\Pages\Service::deletePage($pageId);
    }
    ...
    {
        $result = \Construct\Internal\Pages\Service::getMenus($languageCode);
        $objectArray = array();
        foreach ($result as $menuData)
        {
            $objectArray[] = new \Construct\Page($menuData);
        }
        return $objectArray;
    }
    ...
    {
        $result = \Construct\Internal\Pages\Service::getMenu($languageCode, $alias);
        if ($result) {
            return new \Construct\Page($result);
        }
        return $result;
    }
    ...
    {
        return \Construct\Internal\Pages\Service::createMenu($languageCode, $alias, $title, $type);
    }
